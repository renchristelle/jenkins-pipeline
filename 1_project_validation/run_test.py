...
def test_coding_recipes_complexity(params):
    ...
    for recipe in recipes:
        if recipe["type"] == "python":
            ...
            print(code_analysis)
            assert code_analysis.loc < 1000
            assert code_analysis.lloc < 500
            v = cc_visitors.ComplexityVisitor.from_code(payload)
            assert v.complexity < 210, "Code complexity of recipe " + recipe["name"] + " is too complex: " + v.complexity + " > max value (210)"
